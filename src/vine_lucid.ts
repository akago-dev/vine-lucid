import {
  ...
  mapToObject,
  subpathsObject,
} from "#akago_backend/lib/utils";
import { EnabledRelation } from "#akago_backend/services/model/relations";
...
type VineLucidReturnNullable =
  | VineLucidReturn
  | NullableModifier<VineLucidReturn>;
...
declare module "@vinejs/vine" {
  interface Vine {
    ...
    ): typeof options extends { null: true }
      ? VineLucidReturnNullable
      : VineLucidReturn;
  }
}
...
const vineLucid = function <M extends LucidModel>(
  ...
  const optionsRelationsFirstOrderNames = firstOrderPaths(
    options?.relations ?? [],
    (v) => v.name,
  );

  const relationsProperties = _.mapValues(
    mapToObject(model.$relationsDefinitions),
    (v, k) => {
      // Keep only relation present in options.relations
      if (!optionsRelationsFirstOrderNames.includes(k)) return undefined;
      const optionRelation = _.find(
        options?.relations ?? [],
        (v) => v.name === k,
      );

      // Remove readonly relations if update is true
      if (options?.update && optionRelation?.mutability === "readOnly")
        return undefined;
      ...
      if (meta?.model && meta?.vine) {
        ...
        const forwardedRelationOptions: VineLucidModelOptions = {
          ...
          primaryKey:
            options?.update && optionRelation?.mutability === "assignOnly"
              ? "only"
              : "keep",
  ...
  // Exclude
  const excludes: string[] = options?.update
    ? (model as any).excludeUpdate
    : [];
  ...
  if (!options?.partial) {
    ...
    if (sks?.length && arrayIncludesArray(_.keys(properties), [pk, ...sks])) {
      if (pk in properties)
        properties[pk] = properties[pk].optional().requiredIfAnyMissing(sks);
      for (var sk of sks)
        if (sk in properties) {
...
const vineIsArray = (vine: any): boolean =>
  vine[symbols.UNIQUE_NAME] === "vine.array";
